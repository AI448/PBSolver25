    ...
    pub fn find(
        ...
        for (literal, coefficient) in conflict_constraint.iter_terms() {
            if engine.is_false(literal) {
                ...
                self.falsified_literals.push(Term {
                    literal,
                    coefficient,
                    order,
                });
        ...
        self.falsified_literals
            .sort_unstable_by_key(|term| term.order);

        while self
            .falsified_literals
            .last()
            .is_some_and(|term| sup + term.coefficient < conflict_constraint.lower())
        {
